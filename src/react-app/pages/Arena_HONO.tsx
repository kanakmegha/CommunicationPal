...
// Backend API URL - change to your Hono server URL
...
export default function ArenaPage() {
  ...
  useEffect(() => {
    const checkBackend = async () => {
      try {
        const res = await fetch(`${API_BASE_URL}/api/sessions`, { method: 'GET' });
        if (res.ok) {
          setBackendStatus('online');
          console.log('[Arena] ✅ Backend is online');
        } else {
          setBackendStatus('offline');
        }
      } catch (e) {
        setBackendStatus('offline');
        console.log('[Arena] ❌ Backend is offline');
      }
    };
    checkBackend();
  }, []);
  ...
  /**
   * Stream AI response from Hono backend
   */
  const getAIResponse = useCallback(async (userMessage: string, fillerWords: { word: string; count: number }[]) => {
    console.log('[Arena] Getting AI response for:', userMessage);
    
    // Build message with context
    let enhancedMessage = userMessage;
    if (fillerWords.length > 0) {
      enhancedMessage += `\n\n[Speech note: I used filler words: ${fillerWords.map(f => `"${f.word}" (${f.count}x)`).join(', ')}]`;
    }
    ...
    try {
      ...
      const response = await fetch(`${API_BASE_URL}/api/chat/stream`, {
        ...
        body: JSON.stringify({
          ...
          focus_parameters: activeParams.filter(p => p !== 'swap_list')
        })
      });
      ...
      const aiText = fullResponse || "Tell me more about that.";
      console.log('[Arena] AI response:', aiText);
      ...
      console.error('[Arena] Backend error:', error.message);
      
      // Fallback: try direct Gemini from frontend
      try {
        const { createCoachSession, sendCoachMessage } = await import('@/react-app/lib/gemini');
        const visibleParams = activeParams.filter((p) => p !== "swap_list");
        const session = createCoachSession(visibleParams);
        const aiText = await sendCoachMessage(session, enhancedMessage);
        ...
        console.error('[Arena] Fallback also failed:', e);
      }
  ...
  /**
   * Live analysis via backend
   */
  const analyzeLiveViaBackend = useCallback(async (text: string) => {
    try {
      const response = await fetch(`${API_BASE_URL}/api/analyze-live`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
          text,
          previousScores: {
            articulation: scores.articulation.current,
            expression: scores.expression.current
          },
          focus_parameters: activeParams.filter(p => p !== 'swap_list')
        })
      });
      ...
      if (response.ok) {
        ...
        console.log('[Arena] Backend analysis:', analysis);
        return analysis;
      }
    } catch (e) {
      console.log('[Arena] Backend analysis failed, using local');
    }
    return null;
  }, [scores, activeParams]);
  ...
  const {
    ...
    if (feedback.transcript) {
      ...
      // Analyze articulation locally
      const articulation = analyzeArticulation(userText);
      
      // Try backend analysis as well
      analyzeLiveViaBackend(userText);
      ...
      // Call AI
      getAIResponse(userText, fillerWords);
    ...
    try {
      await fetch(`${API_BASE_URL}/api/sessions`, {
        ...
        body: JSON.stringify({
          ...
          feedback,
          focus_parameters: visibleParams
        })
      });
      console.log('[Arena] Session saved to backend');
    } catch (e) {
      console.log('[Arena] Failed to save session to backend');
    }
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="p-4 border-b border-border flex items-center justify-between shrink-0">
        ...
        {/* Backend Status */}
        <div className="flex items-center gap-2">
          <div className={`px-2 py-1 rounded text-xs ${backendStatus === 'online' ? 'bg-emerald-500/10 text-emerald-500' : backendStatus === 'offline' ? 'bg-destructive/10 text-destructive' : 'bg-muted text-muted-foreground'}`}>
      ...
      {backendStatus === 'offline' && (
        <div className="bg-amber-500/10 border-b border-amber-500/20 p-2 text-center text-amber-600 text-sm">
          ⚠️ Backend server is offline. Start your Hono server or set <code className="bg-amber-500/20 px-1 rounded">VITE_API_URL</code>
        </div>
      )}
