...
export default function ArenaPage() {
  ...
  // ✅ Coach session ref - uses your gemini.ts
  const coachSessionRef = useRef<ReturnType<typeof createCoachSession>>(null);
  ...
  // Initialize coach session on mount using your gemini.ts
  useEffect(() => {
    const visibleParams = activeParams.filter((p) => p !== "swap_list");
    coachSessionRef.current = createCoachSession(visibleParams);
    console.log('[Arena] Coach session created:', coachSessionRef.current ? 'success' : 'failed - check API keys');
  }, [activeParams]);

  // AI Response function - uses your gemini.ts sendCoachMessage
  const getAIResponse = useCallback(async (userMessage: string, fillerWords: { word: string; count: number }[]) => {
    ...
    if (fillerWords.length > 0) {
      enhancedMessage += `\n\n[Speech note: I used these filler words: ${fillerWords.map(f => `"${f.word}" (${f.count}x)`).join(', ')}]`;
    }
    
    try {
      // ✅ Use your existing gemini.ts function
      const aiText = await sendCoachMessage(coachSessionRef.current, enhancedMessage);
  ...
  function getLocalFallback(message: string, fillerWords: { word: string; count: number }[]): string {
    ...
    if (lowerMsg.includes('layoff') || lowerMsg.includes('layoffs') || lowerMsg.includes('fired')) {
      return "I'm sorry about the layoffs happening - that's been tough for many. How is this affecting you?";
    }
  ...
  const handleEndSession = async () => {
    ...
    // ✅ Uses your existing generateGeminiFeedback
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      <header className="p-4 border-b border-border flex items-center justify-between shrink-0">
        ...
        {/* AI Status */}
        <div className="flex items-center gap-2">
          <div className={`px-2 py-1 rounded text-xs ${coachSessionRef.current ? 'bg-emerald-500/10 text-emerald-500' : 'bg-amber-500/10 text-amber-500'}`}>
            {coachSessionRef.current ? '🟢 AI Ready' : '🟡 Check API Keys'}
          </div>
        </div>
